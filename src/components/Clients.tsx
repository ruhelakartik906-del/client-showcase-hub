      ...
      <div className="relative mt-12 overflow-hidden [mask-image:linear-gradient(to_right,transparent,black_8%,black_92%,transparent)]">
        <div className="marquee-track flex w-max gap-4 hover:[animation-play-state:paused]">
          {row.map((c, i) => (
            <div
              key={`${c.name}-${i}`}
              className="group relative flex h-24 w-44 shrink-0 items-center justify-center rounded-xl border border-white/8 bg-white/[0.02] px-6 transition-all duration-500 hover:border-brand/40 hover:bg-white/[0.05]"
            >
              <img
                src={c.logo}
                alt={`${c.name} logo`}
                loading="lazy"
                className="max-h-12 w-auto max-w-[7rem] object-contain opacity-45 grayscale transition-all duration-500 group-hover:scale-105 group-hover:opacity-100 group-hover:grayscale-0"
              />
